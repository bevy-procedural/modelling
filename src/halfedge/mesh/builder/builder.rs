...
impl<T: HalfEdgeImplMeshTypePlus> MeshBuilder<T> for HalfEdgeMeshImpl<T> {
    ...
    fn insert_edge_ee(&mut self, input: T::E, output: T::E, ep: T::EP) -> Option<T::E> {
        ...
        debug_assert!(self.edge(e1).is_valid(self));
        debug_assert!(self.edge(e2).is_valid(self));
    ...
    fn insert_edge_vv(&mut self, a: T::V, b: T::V, ep: T::EP) -> Option<T::E> {
        ...
        if av.is_isolated(self) {
            if bv.is_isolated(self) {
                ...
                let Some(a_in) = av.ingoing_boundary_edge(self) else {
                    return None;
                };
                let (e1, e2) = self.insert_halfedge_pair_forced(
                    a_in,
                    a,
                    self.edge(a_in).next_id(),
                    IndexType::max(),
                    b,
                    IndexType::max(),
                    IndexType::max(),
        ...
        } else if bv.is_isolated(self) {
            // a is not isolated, b is isolated

            // find a unique boundary edge
            let Some(b_in) = bv.ingoing_boundary_edge(self) else {
                return None;
            };
            let (e1, e2) = self.insert_halfedge_pair_forced(
                IndexType::max(),
            ...
            if !ep.allow_multigraph() {
                if let Some(duplicate) = self.shared_edge(a, b) {
                    if !self.edge_payload(&duplicate).allow_multigraph() {
                        return None;
                    }
                }
            ...
            for e in av.edges_out(self) {
                ...
                if let Some(other_e) = e.same_boundary(self, b) {
                    if m.is_some() {
                        return None;
                    }
                    m = Some((e.id(), other_e));
            ...
            return self.insert_edge_ee(from_a, self.edge(from_b).prev_id(), ep);
        }
    }
