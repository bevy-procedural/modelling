...
pub fn show_vertex_indices(texts: &mut ResMut<Text3dGizmos>, mesh: &MeshVec3) {
    mesh.vertices().for_each(|v| {
        texts.write(
            Text3dGizmo::new(v.id().to_string(), v.vertex().clone()).with_color(Color::BLUE),
        );
    });
}
...
pub fn show_edges(texts: &mut ResMut<Text3dGizmos>, mesh: &MeshVec3, offset: f32) {
    mesh.edges().for_each(|e| {
        if let Some(f) = e.face(mesh) {
            ...
            texts.write(Text3dGizmo::new(e.id().to_string(), p01).with_color(Color::YELLOW));
        } else {
            texts.write(
                Text3dGizmo::new(e.id().to_string(), e.center(mesh).clone()).with_color(Color::RED),
            );
        }
    });
}
...
pub fn show_faces(texts: &mut ResMut<Text3dGizmos>, mesh: &MeshVec3) {
    mesh.faces().for_each(|f| {
        texts.write(Text3dGizmo::new(f.id().to_string(), f.center(mesh).clone()).with_color(Color::GREEN));
    });
}