    ...
    T::FP: DefaultFacePayload,
{
    ...
    pub fn cuboid(x: T::S, y: T::S, z: T::S) -> Mesh<T> {
        ...
        let v2 = mesh.add_vertex_via_vertex(v1, make(-x, -y, z), ep, ep).0;
        let v3 = mesh.add_vertex_via_vertex(v2, make(x, -y, z), ep, ep).0;
        mesh.close_face_vertices(v2, ep, v3, ep, v0, fp, false);
        let v4 = mesh.add_vertex_via_vertex(v1, make(-x, y, -z), ep, ep).0;
        let v5 = mesh.add_vertex_via_vertex(v4, make(-x, -y, -z), ep, ep).0;
        mesh.close_face_vertices(v4, ep, v5, ep, v2, fp, false);
        let v6 = mesh.add_vertex_via_vertex(v0, make(x, y, -z), ep, ep).0;
        let v7 = mesh.add_vertex_via_vertex(v3, make(x, -y, -z), ep, ep).0;
