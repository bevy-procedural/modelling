    ...
    P: Payload + Vector3D<P::S>,
{
    ...
    pub fn cuboid(x: P::S, y: P::S, z: P::S) -> Mesh<E, V, F, P> {
        ...
        let v2 = mesh.add_vertex((v1, P::from_xyz(-x, -y, z))).0;
        let v3 = mesh.add_vertex((v2, P::from_xyz(x, -y, z))).0;
        mesh.close_face((v2, v3, v0));
        let v4 = mesh.add_vertex((v1, P::from_xyz(-x, y, -z))).0;
        let v5 = mesh.add_vertex((v4, P::from_xyz(-x, -y, -z))).0;
        mesh.close_face((v4, v5, v2));
        let v6 = mesh.add_vertex((v0, P::from_xyz(x, y, -z))).0;
        let v7 = mesh.add_vertex((v3, P::from_xyz(x, -y, -z))).0;
